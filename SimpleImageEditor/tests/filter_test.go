		...
		func() {
			...
			go func() {
				...
				for y := 0; y < neg.Image.Bounds().Max.Y; y++ {
					...
					if pixel2.Max8BitPixelColor-shiftOrigR != shiftNewR {
						T.Error(errors.New(fmt.Sprintf("R diferente em (%d, %d),\t\tesperado: %d, obtido: %d", x, y, pixel2.Max8BitPixelColor-shiftOrigR, shiftNewR)))
					}

					if pixel2.Max8BitPixelColor-shiftOrigG != shiftNewG {
						T.Error(errors.New(fmt.Sprintf("G diferente em (%d, %d),\t\tesperado: %d, obtido: %d", x, y, pixel2.Max8BitPixelColor-shiftOrigG, shiftNewG)))
					}

					if pixel2.Max8BitPixelColor-shiftOrigB != shiftNewB {
						T.Error(errors.New(fmt.Sprintf("B diferente em (%d, %d),\t\tesperado: %d, obtido: %d", x, y, pixel2.Max8BitPixelColor-shiftOrigB, shiftNewB)))
					}
				}
			}()
		...
		func() {
			...
			go func() {
				...
				for y := 0; y < YIQ.Image.Bounds().Max.Y; y++ {
					old := egdImage.YIQModel(YIQ.Image.At(x, y))
					newI := neg.Image.At(x, y)

					if n, o := uint(math.Round(newI.(pixel2.YIQ).Y)), uint(math.Round(pixel2.Max8BitPixelColor-old.(pixel2.YIQ).Y)); n != o {
						T.Error(errors.New(fmt.Sprintf("Y diferente em (%d, %d),\t\tesperado: %d, obtido: %d",
							x, y, pixel2.Max8BitPixelColor-o, n)))
					}

					if n, o := int(math.Round(neg.Image.At(x, y).(pixel2.YIQ).I)), int(math.Round(old.(pixel2.YIQ).I)); n != o {
						T.Error(errors.New(fmt.Sprintf("I diferente em (%d, %d),\t\tesperado: %d, obtido: %d",
							x, y, pixel2.Max8BitPixelColor-o, n)))
					}

					if n, o := int(math.Round(neg.Image.At(x, y).(pixel2.YIQ).Q)), int(math.Round(old.(pixel2.YIQ).Q)); n != o {
...
func TestMedian(T *testing.T) {
	...
	yiq := egdImage.NewYIQ(10, 10)
	for i := 0; i < 10; i++ {
		for j := 0; j < 10; j++ {
			yiq.Set(i, j, pixel2.YIQ{Y: float64(i + j)})
		}
	}

	im := egdImage.Image{Image: yiq}
	im.PixelFormat = egdImage.PixelFormatYIQ
	...
	for i := 0; i < 10; i++ {
		for j := 0; j < 10; j++ {
			if im.Image.At(i, j).(pixel2.YIQ).Y != expected[i*10+j] {
				T.Errorf("expected: %f; got: %f", expected[i*10+j], im.Image.At(i, j).(pixel2.YIQ).Y)
			}
		}
	}
}
